...
static void read_cb(uv_stream_t* client, ssize_t nread, const uv_buf_t* buf) {
  ...
  r = uv_write((uv_write_t *) &write_req->req, (uv_stream_t*) client, &write_req->buf, 1, write_cb);
  CHECK(r, "uv_write");
}
...
int main() {
  ...
  r = uv_ip4_addr(HOST, PORT, (struct sockaddr*) &addr);
  CHECK(r, "uv_ip4_addr");
  ...
  r = uv_listen((uv_stream_t*) &tcp_server, 5, onconnection);
  CHECK(r, "uv_listen");
